...
const NeighborhoodSelect = () => {
  ...
  useEffect(() => {
    const selectedNeighborhood = neighborhoods.find(
      (n) => n.idNeighborhood === Number(selectedNeighborhoodID)
    );
    if (!selectedNeighborhood) return;
    setSelectedNeighborhood({
      value: selectedNeighborhood.idNeighborhood,
      label: selectedNeighborhood.neighborhoodName,
    });
    ...
    <Flex gap={2} alignItems={"center"} justify={"center"} zIndex={5000}>
      <Select
        ...
        options={neighborhoods.map((v) => {
          return { value: v.idNeighborhood, label: v.neighborhoodName };
        })}
      />
    </Flex>
  );
